...
export class Semaphore {
  ...
  /**
   * Optionally enters the semaphore to call an function, leaving it upon function completion.
   * This ensures the semaphore is left, even if the function throws an error.
   * This will not enter the semaphore and not call the function if there was not capacity to do that.
   * @template TReturn
   * @param {function(...):TReturn} func - Function to be executed
   * @param [thisArg] The object to be used as the current object.
   * @param [args] - Arguments to be passed to the method.
  ...
  /**
   * Optionally enters the semaphore to call and await an async function, leaving it upon function/promise completion.
   * This ensures the semaphore is left, even if the function throws an error.
   * This will not enter the semaphore and not call the function if there was not capacity to do that.
   * @template TReturn
   * @param {function(...):Promise<TReturn>} func - Function to be executed
   ...
   * @return {TReturn}
   */
  async tryCallAsync (func, thisArg, ...args) {
    if (this.hasFreeCapacity) {
      return this.callAsync(func, thisArg, ...args)
    }
  }
}
